...
// Du lieu cho trong file QHD_CaiBalo2.txt
...
typedef struct {
	char TenDV[20];
	float  GT;
	int  SL, TL, PA; //PA la so do vat duoc chon
} DoVat;
...
//dien vao hang dau tien cua bang
	...
	for(V=0; V<=W; V++){
		X[0][V] = V/dsdv[0].TL;	// X la trong luong do mang bat dau tu 0 nen [0][V] = [1][V]
...
// dien cac dong con lai
	for(k=1; k<n; k++)
		...
		//	yk = V/dsdv[k].TL; // ba lo 1	QHD
		  ...
		  yk = min(dsdv[k].SL, V/dsdv[k].TL); 	// ba lo 2 QHD:
			
		// yk = min(1, V/dsdv[k].TL);    //	ba lo 3 QHD: 
